...
func (p *Project) Execute(path string) error {
	...
	err = generate("bootstrap/src/check_main.go", path, filepath.Join("cmd", "check", "main.go"), p)
	if err != nil {
		return err
	}

	os.MkdirAll(filepath.Join(path, "cmd", "get"), os.ModePerm)
	err = generate("bootstrap/src/get_main.go", path, filepath.Join("cmd", "get", "main.go"), p)
	if err != nil {
		return err
	}

	os.MkdirAll(filepath.Join(path, "cmd", "put"), os.ModePerm)
	err = generate("bootstrap/src/put_main.go", path, filepath.Join("cmd", "put", "main.go"), p)
	if err != nil {
		return err
	}
...
func generate(src, dest, name string, p *Project) error {
